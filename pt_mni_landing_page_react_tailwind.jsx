...
function KBadge({ children }: { children: React.ReactNode }) {
  return (
...
export default function LandingPageMNI() {
  return (
    <div className="min-h-screen bg-white text-[#2D3748] [font-family:var(--font-body,Lato,sans-serif)]">
              ...
              {[1, 2, 3, 4, 5, 6].map((i) => (
                <div key={i} className="aspect-[4/5] rounded-2xl overflow-hidden bg-[#F5F7FA] border border-[#E2E8F0]">
                  <img
                    ...
                    alt="Alumni placeholder — Ganti dengan foto asli alumni/kegiatan"
      ...
      {/* Masalah & Solusi */}
      <section className="bg-[#F5F7FA]" id="masalah">
      ...
      <section id="keunggulan">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-14">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {[
              { icon: <ShieldCheck className="w-5 h-5 text-[#BC002D]" />, text: "Legalitas jelas & transparan" },
              { icon: <BookOpen className="w-5 h-5 text-[#BC002D]" />, text: "Kurikulum bahasa & etos kerja standar industri" },
              { icon: <HeartHandshake className="w-5 h-5 text-[#BC002D]" />, text: "Pembinaan spiritual & komunitas Muslim" },
              { icon: <Users className="w-5 h-5 text-[#BC002D]" />, text: "Mentor/alumni sebagai pendamping karier" },
              { icon: <Clock className="w-5 h-5 text-[#BC002D]" />, text: "Timeline terukur hingga keberangkatan" },
              { icon: <MapPin className="w-5 h-5 text-[#BC002D]" />, text: "Jaringan penempatan di berbagai prefektur" },
            ].map((it, i) => (
              <div key={i} className="flex items-start gap-3 p-4 rounded-2xl border border-[#E2E8F0] bg-white">
                {it.icon}
                <p className="text-sm text-[#2D3748]">{it.text}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* Program Pelatihan */}
      <section className="bg-[#F5F7FA]" id="program">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-14">
          <SectionTitle
          ...
          <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-6">
            {/* Kaigo — Unggulan */}
            <Card className="border-[#E2E8F0] ring-2 ring-[#BC002D]/20">
              <CardHeader>
                <div className="flex items-center justify-between">
                  <CardTitle className="text-[#1A2B48] flex items-center gap-2"><HeartHandshake className="w-5 h-5 text-[#BC002D]" /> Kaigo (Caregiver) — Unggulan</CardTitle>
                  <span className="text-xs px-2 py-1 rounded-full bg-[#BC002D]/10 text-[#BC002D]">Banyak Demand</span>
                </div>
              </CardHeader>
              <CardContent className="text-sm text-[#2D3748]/80 space-y-3">
                <ul className="list-disc pl-5 space-y-1">
                  <li>Fokus bahasa Jepang + keterampilan caregiving & budaya kerja.</li>
                  <li>Simulasi kerja, SOP fasilitas lansia, etika & komunikasi.</li>
                  <li>Pendampingan sertifikasi sesuai skema penempatan.</li>
                </ul>
                <div className="flex items-center gap-4 text-xs">
                  <span className="inline-flex items-center gap-1"><Clock className="w-4 h-4" /> 4–8 bulan</span>
                  <span className="inline-flex items-center gap-1"><Users className="w-4 h-4" /> Min. SMA/SMK</span>
                </div>
                <div className="pt-2">
                  <Button asChild className="bg-[#BC002D] hover:bg-[#BC002D]/90 w-full">
                    <a href={WA_LINK} target="_blank" rel="noreferrer">Tanya Detail Kaigo</a>
                  </Button>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="border-[#E2E8F0]">
              <CardHeader>
                <CardTitle className="text-[#1A2B48] flex items-center gap-2"><GraduationCap className="w-5 h-5 text-[#BC002D]" /> Magang Jepang (Internship)</CardTitle>
              </CardHeader>
              <CardContent className="text-sm text-[#2D3748]/80 space-y-3">
                <ul className="list-disc pl-5 space-y-1">
                  <li>Pelatihan bahasa & teknis dasar sesuai bidang.</li>
                  <li>Pendampingan dokumen & interview.</li>
                </ul>
                <div className="flex items-center gap-4 text-xs">
                  <span className="inline-flex items-center gap-1"><Clock className="w-4 h-4" /> 3–6 bulan</span>
                  <span className="inline-flex items-center gap-1"><Factory className="w-4 h-4" /> Beragam sektor</span>
                </div>
                <div className="pt-2">
                  <Button asChild variant="outline" className="border-[#BC002D] text-[#BC002D] hover:bg-[#BC002D]/10 w-full">
                    <a href={WA_LINK} target="_blank" rel="noreferrer">Tanya Program Magang</a>
                  </Button>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="border-[#E2E8F0]">
              <CardHeader>
                <CardTitle className="text-[#1A2B48] flex items-center gap-2"><ChefHat className="w-5 h-5 text-[#BC002D]" /> SSW (Specified Skilled Worker)</CardTitle>
              </CardHeader>
              <CardContent className="text-sm text-[#2D3748]/80 space-y-3">
                <ul className="list-disc pl-5 space-y-1">
                  <li>Persiapan bahasa + materi sesuai sektor (mis. food service/manufacturing).</li>
                  <li>Simulasi ujian & pendampingan dokumen.</li>
                </ul>
                <div className="flex items-center gap-4 text-xs">
                  <span className="inline-flex items-center gap-1"><Clock className="w-4 h-4" /> 4–8 bulan</span>
                  <span className="inline-flex items-center gap-1"><Users className="w-4 h-4" /> Lulusan SMA/SMK/D3/S1</span>
                </div>
                <div className="pt-2">
                  <Button asChild variant="outline" className="border-[#BC002D] text-[#BC002D] hover:bg-[#BC002D]/10 w-full">
                    <a href={WA_LINK} target="_blank" rel="noreferrer">Tanya Program SSW</a>
                  </Button>
                </div>
              </CardContent>
            </Card>
      ...
      <section id="testimoni">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-14">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[1, 2, 3].map((i) => (
              <Card key={i} className="border-[#E2E8F0] overflow-hidden">
                <div className="relative aspect-video bg-[#F5F7FA]">
                  <img src={`https://picsum.photos/seed/testi${i}/800/450`} alt="Video testimoni placeholder" className="w-full h-full object-cover" />
      ...
      <section className="bg-[#F5F7FA]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-14">
          ...
          <ol className="grid md:grid-cols-5 sm:grid-cols-3 grid-cols-2 gap-5">
            {[
              { icon: <MessageCircle className="w-5 h-5 text-[#BC002D]" />, title: "Konsultasi Awal" },
              { icon: <BookOpen className="w-5 h-5 text-[#BC002D]" />, title: "Daftar & Pelatihan" },
              { icon: <Handshake className="w-5 h-5 text-[#BC002D]" />, title: "Dokumen & Interview" },
              { icon: <ShieldCheck className="w-5 h-5 text-[#BC002D]" />, title: "Penempatan" },
              { icon: <PlaneTakeoff className="w-5 h-5 text-[#BC002D]" />, title: "Keberangkatan" },
            ].map((step, idx) => (
              <li key={idx} className="p-4 rounded-2xl bg-white border border-[#E2E8F0] flex flex-col items-start gap-3">
                <div className="w-8 h-8 rounded-full bg-[#BC002D]/10 flex items-center justify-center">{step.icon}</div>
      ...
      {/* Final CTA */}
      <section id="kontak" className="bg-[#F5F7FA]">
          ...
          <div className="rounded-3xl border border-[#E2E8F0] bg-white p-6 sm:p-10">
            ...
            >
              ...
              <div className="sm:col-span-2 flex flex-col sm:flex-row gap-3 mt-2">
                <Button type="submit" className="bg-[#BC002D] hover:bg-[#BC002D]/90 h-11">Kirim & Konsultasi via WhatsApp</Button>
                <Button asChild variant="outline" className="border-[#BC002D] text-[#BC002D] hover:bg-[#BC002D]/10 h-11">
